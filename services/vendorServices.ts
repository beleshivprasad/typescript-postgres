...
export const addItem = async (req: Request): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const found = await productModel.view(
        QUERY.PRODUCT_EXISTS,
        req.body.product_name,
        req.body.brand_name
      );
      if (found.rows.length === 0) {
        const result: boolean | String = await productModel.create(
      ...
      reject(
        `Product already Exists with name ${req.body.product_name} and branch ${req.body.brand_name} `
      );
    } catch (error: any) {
      reject(error);
    }
...
export const updateItem = async (req: Request): Promise<boolean | String> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let cat: string = "";
      let subcat: string = "";
      let product: object[] = await productModel.view(
        QUERY.VIEW_PRODUCT,
        req.params.id
      );
      if (product.length === 0) {
...
export const showItem = async (req: Request): Promise<any> => {
  return new Promise(async (resolve, reject) => {
    try {
      const result: any = await productModel.view(
        QUERY.VIEW_PRODUCT,
        req.params.id
      );
      resolve(result);
...
export const deleteItem = async (req: Request): Promise<boolean | String> => {
  return new Promise(async (resolve, reject) => {
    try {
      console.log(req.params.id);
      const product = await productModel.view(
        QUERY.VIEW_PRODUCT,
        req.params.id
      );
      if (product.length === 0) {
